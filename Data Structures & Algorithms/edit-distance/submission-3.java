class Solution {
    ...
    private int dp(String word1, String word2, int i, int j, Integer[][] memo) {
        ...
        if (word1.charAt(i) == word2.charAt(j)) {
            ...
            // replace: fix mismatch at (i,j), advance both pointers       -> dp(i+1, j+1)
            // delete:  remove word1[i], advance only i                    -> dp(i+1, j)
            // insert:  insert word2[j] into word1, advance only j         -> dp(i, j+1)
            var replace = dp(word1, word2, i + 1, j + 1, memo);
            var delete  = dp(word1, word2, i + 1, j,     memo);
            var insert  = dp(word1, word2, i,     j + 1, memo);
