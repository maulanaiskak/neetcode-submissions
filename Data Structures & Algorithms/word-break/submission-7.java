class Solution {
    public boolean wordBreak(String s, List<String> wordDict) {
        ...
        // optim: use HashSet for O(1) lookup, prune start by maxWordLen to skip impossible substrings

        var wordSet = new HashSet<>(wordDict);
        var maxWordLen = wordDict.stream().mapToInt(String::length).max().orElse(0);

        var canBreak = new boolean[s.length() + 1];
        canBreak[0] = true;

        for (var end = 1; end <= s.length(); end++) {
            for (var start = Math.max(0, end - maxWordLen); start < end; start++) {
                var lastWord = s.substring(start, end);
                var prefixIsBreakable = canBreak[start];
                var lastWordInDict = wordSet.contains(lastWord);
